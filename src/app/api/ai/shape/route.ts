...
Generate the complete structured JSON response.`;
    ...
    if (!rawJsonText) {
      return NextResponse.json(
        {
          success: false,
          error: 'NEXUS AI returned an empty response for brand shaping. Please retry.',
        },
        { status: 502 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'NEXUS AI returned an unparseable response for brand shaping. Please retry.',
        },
        { status: 502 }
      );
    }
